...
int main(int argc, char **argv)
{
    ...
    for(size_t ni = 0; ni < nImages; ni++)
    {
        ...
        if(ni < nImages-1)
            T = (1.0/30.0)*(timestamps[ni+1] - tframe);
        else if(ni>0)
            T =  (1.0/30.0)*(tframe - timestamps[ni-1]);
...
{
    ifstream times;
    string pathToTimeFile = pathToSequence + "/associations.txt";
    ...
    while(!times.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            ORB_SLAM2::Seconds tDepth;
            ss >> tDepth;

            string sDepth;
            ss >> sDepth;

            ORB_SLAM2::Seconds tRgb;
            ss >> tRgb;
            timestamps.push_back(tRgb);
