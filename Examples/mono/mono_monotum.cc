...
int main(int argc, char **argv)
{
    ...
    ORB_SLAM2::System SLAM(path_to_vocabulary,path_to_settings,
                           ORB_SLAM2::System::MONOCULAR,activateVisualization);
    ...
    SLAM.SaveKeyFrameTrajectoryTUM(resultsPath + "_FrameTrajectory.txt");
...
void LoadImages(const string &pathToSequence, vector<string> &imageFilenames, vector<ORB_SLAM2::Seconds> &timestamps)
{
    ...
    while(!times.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            string sRGB;
            ss >> sRGB;
            imageFilenames.push_back(pathToSequence + "/undistortedImages/" + sRGB + ".jpg");
            ss >> t;
