...
void KeyFrame::ComputeBoW()
{
    ...
    {
        vector<DESCRIPTOR_FORMAT> vCurrentDesc = Converter::toDescriptorVector(mDescriptors);
...
{
    unique_lock<mutex> lock(mMutexConnections);
    map<KeyframeId,Keyframe> s;
    for(auto mit: mConnectedKeyFrames)
        s[mit.first] = mit.second;
    return s;
}
...
MapPoint* KeyFrame::CreateMapPoint(const cv::Mat& worldPos, const KeypointIndex& projIndex)
{
...
void KeyFrame::AddMapPoint(MapPt mapPt, const KeypointIndex &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapPoints[idx] = mapPt;

    if(this == mapPt->GetCurrentRefKeyframe())
        mapPt->SetCurrentRefKeyframeIndex(idx);
}
...
std::map<long unsigned int ,MapPoint*> KeyFrame::GetMapPoints()
{
    unique_lock<mutex> lock(mMutexFeatures);
    std::map<long unsigned int ,MapPoint*> s;
...
void KeyFrame::UpdateConnections()
{
    ...
    for(vector<MapPoint*>::iterator vit=vpMP.begin(), vend=vpMP.end(); vit!=vend; vit++)
    {
        ...
        for(auto& mit: observations)
        {
            if(mit.first == mnId)
                continue;
            KFweightsCounter[mit.first]++;
            KFcounter[mit.first] = mit.second.projKeyframe;
        }
    }
    ...
    for(auto& mit: KFweightsCounter)
    {
        ...
        if(mit.second>=th)
        {
            vPairs.push_back(make_pair(mit.second,KFcounter[mit.first]));
...
map<KeyframeId , KeyFrame*> KeyFrame::GetChilds()
{
    unique_lock<mutex> lockCon(mMutexConnections);
    return mspChildrens;
}
...
std::map<KeyframeId, KeyFrame*> KeyFrame::GetLoopEdges()
{
    unique_lock<mutex> lockCon(mMutexConnections);
    return mspLoopEdges;
}
...
{   
    ...
    for(auto& connectedKeyFrame: mConnectedKeyFrames)
        connectedKeyFrame.second->EraseConnection(this);
...

    {
        ...
        map<KeyframeId, Keyframe*> sParentCandidates;
        ...
        while(!mspChildrens.empty())
        {
            ...
            for(auto& keyframeTmp: mspChildrens)
            {
                ...
                vector<Keyframe> vpConnected = keyframe->GetVectorCovisibleKeyFrames();
                for(size_t i = 0, iend = vpConnected.size(); i < iend; i++)
                {
                    for(auto spcit:sParentCandidates)
                    {
                        if(vpConnected[i]->mnId == spcit.first)
                        {
                            int w = keyframe->GetWeight(vpConnected[i]);
                            if(w>max)
                            {
                                pC = keyframe;
                                pP = vpConnected[i];
        ...
        if(!mspChildrens.empty())
            for(auto& sit: mspChildrens)
            {
                sit.second->ChangeParent(mpParent);
            }
