 ...
 * Description: functions for Akaze16 descriptors
...
// --------------------------------------------------------------------------
    ...
    void DESCRIPTOR_CLASS_NAME::meanValue(const std::vector<DESCRIPTOR_CLASS_NAME::pDescriptor> &descriptors, DESCRIPTOR_CLASS_NAME::TDescriptor &mean)
    {
        ...
        {
            vector<int> sum(DESCRIPTOR_CLASS_NAME::L * 8, 0);
            ...
            mean = cv::Mat::zeros(1, DESCRIPTOR_CLASS_NAME::L, CV_8U);
...
// --------------------------------------------------------------------------
    ...
    double DESCRIPTOR_CLASS_NAME::distance(const DESCRIPTOR_CLASS_NAME::TDescriptor &a,
                                           const DESCRIPTOR_CLASS_NAME::TDescriptor &b)
    {
...
// --------------------------------------------------------------------------
    ...
    {
        a.create(1, DESCRIPTOR_CLASS_NAME::L, CV_8U);
        unsigned char *p = a.ptr<unsigned char>();

        stringstream ss(s);
        for(int i = 0; i < DESCRIPTOR_CLASS_NAME::L; ++i, ++p)
        {
            int n;
            ss >> n;
...
// --------------------------------------------------------------------------
                                         ...
                                         cv::Mat &mat)
    {
        ...
        mat.create(N, DESCRIPTOR_CLASS_NAME::L*8, CV_32F);
...
// --------------------------------------------------------------------------
    ...
    void DESCRIPTOR_CLASS_NAME::toMat32F(const cv::Mat &descriptors, cv::Mat &mat)
    {
...
// --------------------------------------------------------------------------
    ...
    void DESCRIPTOR_CLASS_NAME::toMat8U(const std::vector<TDescriptor> &descriptors,
                                        cv::Mat &mat)
    {
        mat.create(descriptors.size(), DESCRIPTOR_CLASS_NAME::L, CV_8U);

        unsigned char *p = mat.ptr<unsigned char>();

        for(size_t i = 0; i < descriptors.size(); ++i, p += DESCRIPTOR_CLASS_NAME::L)
        {
            const unsigned char *d = descriptors[i].ptr<unsigned char>();
            std::copy(d, d + DESCRIPTOR_CLASS_NAME::L, p);
        }
